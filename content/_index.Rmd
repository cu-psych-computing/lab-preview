...
**The Columbia Psychology Lab Preview is scheduled for September 15th, 3:00 - 4:30 pm outside Schermerhorn Hall.**

Click the button below to add the event to your Google Calendar

<a target="_blank" href="https://calendar.google.com/event?action=TEMPLATE&amp;tmeid=MGE5NWlwNjQ2cHJkbmc2ZzBmZ3JuMnVxdGUgY19zNmZzdXU3N2tpMzI3ODBmNjdmMTg1MGI4Z0Bn&amp;tmsrc=c_s6fsuu77ki32780f67f1850b8g%40group.calendar.google.com"><img border="0" src="https://www.google.com/calendar/images/ext/gc_button1_en.gif"></a>
...
This website is meant to act as an asynchronous extension of our typical Lab Preview Day. If you can't make it to the official event, feel free to use the recruitment videos, blurbs, and contact info on this website to reach out to labs you're interested in. The website will be updated throughout the year as the needs of labs change, so even if labs aren't recruiting at the moment they may be in the future.
...
labs <- read_delim("../static/lab-info.txt", delim = "\t", show_col_types = FALSE) %>%
  ...
  mutate("Accepting RAs" = RAs)


options(kableExtra.html.bsTable = TRUE)
labs %>%
  select(Faculty, Lab, "Accepting RAs") %>%
  kbl() %>%
